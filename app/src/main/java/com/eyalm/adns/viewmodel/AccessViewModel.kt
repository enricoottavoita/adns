...
class AccessViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun load() {
        ...
        viewModelScope.launch {
            when (val result = repository.getAccess()) {
    ...
    fun submitInvite() {
        ...
        viewModelScope.launch {
            when (val result = repository.inviteAccess(email, current.inviteRole)) {
    ...
    fun submitRoleChange() {
        ...
        viewModelScope.launch {
            when (repository.updateAccessRole(target.email, current.selectedRole)) {
                is ApiResult.Success -> {
    ...
    fun confirmDelete() {
        ...
        _state.update { it.copy(submitting = true, errorMessage = null) }
        viewModelScope.launch {
            when (repository.deleteAccess(target.email)) {
                is ApiResult.Success -> {
