...
class RecreationViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun load() {
        ...
        viewModelScope.launch {
            when (val result = repository.getParentalRecreation()) {
    ...
    fun saveSchedule() {
        ...
        viewModelScope.launch {
            when (repository.updateRecreationSchedule(schedule)) {
    ...
    fun toggleItem(collection: RecreationItemCollection, item: ParentalRecreationItem) {
        ...
        viewModelScope.launch {
            when (repository.updateRecreationItem(collection, item.id, !item.recreation)) {
                is ApiResult.Success -> {
